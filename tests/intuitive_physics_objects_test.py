...
SIDEWAYS_SHAPES = [
    ...
    'dog_on_wheels_2', 'skateboard', 'jeep', 'tank_1', 'tank_2', 'trike',
    'truck_3', 'truck_4'
]


def test_getters_reuse_immutable_dataset():
    dataset_1 = intuitive_physics_objects.get_fall_down_definition_dataset(unshuffled=True)  # noqa: E501
    dataset_2 = intuitive_physics_objects.get_fall_down_definition_dataset(unshuffled=True)  # noqa: E501
    assert dataset_1 is dataset_2
    assert isinstance(dataset_1, DefinitionDataset)


def test_does_have_definitions():
    dataset = intuitive_physics_objects.get_fall_down_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_fall_down_basic_shape_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_fall_down_complex_shape_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_fall_down_basic_shape_opposite_colors_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_fall_down_complex_shape_opposite_colors_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_move_across_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_move_across_basic_shape_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_move_across_complex_shape_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_move_across_basic_shape_opposite_colors_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)

    dataset = intuitive_physics_objects.get_move_across_complex_shape_opposite_colors_definition_dataset(unshuffled=True)  # noqa: E501
    definitions = dataset.definitions_unique_shape_scale()
    assert len(definitions) > 0
    for definition in definitions:
        assert isinstance(definition, ObjectDefinition)
...
def test_intuitive_physics_move_across_all_objects_untrained_shapes():
    ...
    for definition_1 in trained_dataset.definitions_unique_shape_scale():
        filtered_dataset = untrained_dataset.filter_on_similar_except_shape(
            definition_1,
            only_diagonal_size=True
        )
        assert len(filtered_dataset.definitions_unique_shape_scale()) >= 2
